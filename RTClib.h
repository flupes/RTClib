...
class DateTime {
 public:
  ...
  DateTime(uint16_t year, uint8_t month, uint8_t day, uint8_t hour = 0,
           uint8_t min = 0, uint8_t sec = 0, int8_t tz = 0);
  ...
  // Comparison operators: these operators are computationally
  // expensive because I am lazy and use unixtime() rather
  // than more elaborate comparisons!
  bool operator<(const DateTime& right) const;
  bool operator==(const DateTime& right) const;

  // Returns the day in the year (starting at 1, not 0!)
  uint16_t yDay() const;

  // Returns a localtime constructed from this time
  DateTime getLocalTime(int8_t time_zone_offset) const;

  // Generate a human readable string in the provided buffer
  // Warning: make sure to allocate enough space (~50?)
  void toString(char* buffer) const {
    sprintf(buffer, "%04d-%02d-%02d %02d:%02d:%02d %+d",  // include time zone
            year(), m, d, hh, mm, ss, tzo);
  }
